package com.gether.research.test.stream.flink;

import com.gether.research.test.stream.kafka.case1.PurchaseAnalysis;
...
import com.gether.research.test.stream.kafka.case1.producer.OrderProducer;
...
public class KafkaPurchaseAnalysisTest {
  ...
  public static void main(String[] args) throws Exception {
    ...
    FlinkKafkaConsumer010<String> orderConsumer = new FlinkKafkaConsumer010<String>(
        TOPIC_INPUT_ORDERS, new SimpleStringSchema(), properties);
    FlinkKafkaConsumer010<String> userConsumer = new FlinkKafkaConsumer010<String>(
        TOPIC_INPUT_USERS, new SimpleStringSchema(), properties);
    FlinkKafkaConsumer010<String> itemsConsumer = new FlinkKafkaConsumer010<String>(
        TOPIC_INPUT_ITEMS, new SimpleStringSchema(), properties);
    ...
    DataStream<Tuple2<String, Double>> result = ordersStream
        ...
        .flatMap(new FlatMapFunction<String, Order>() {
          ...
          public void flatMap(String s, Collector<Order> collector) throws Exception {
            ...
            LocalDateTime time = LocalDateTime
                .parse(split[2].trim(), OrderProducer.dataTimeFormatter);
            long ms = time.atZone(ZoneId.systemDefault()).toInstant().toEpochMilli();
            collector.collect(
                new Order(split[0].trim(), split[1].trim(), ms, Integer.valueOf(split[3].trim())));
          }
        })
        .join(
            usersStream.filter(s -> StringUtils.isNotBlank(s) && s.split(",").length == 4)
                .flatMap(new FlatMapFunction<String, User>() {
                  ...
                  public void flatMap(String s, Collector<User> collector) throws Exception {
                    String[] split = s.split(",");
                    collector.collect(new User(split[0].trim(), split[1].trim(), split[2].trim(),
                        Integer.valueOf(split[3].trim())));
                  }
                })
        )
        ...
        .apply((JoinFunction<Order, User, PurchaseAnalysis.OrderUser>) (order, user) -> {
          //System.out.println(LocalDateTime.now().toString() + "_" + JSON.toJSONString(order) + "_" + JSON.toJSONString(user));
          PurchaseAnalysis.OrderUser orderUser = PurchaseAnalysis.OrderUser
            ...
            itemsStream.filter(s -> StringUtils.isNotBlank(s) && s.split(",").length == 4)
                .flatMap(new FlatMapFunction<String, Item>() {
                  ...
                  public void flatMap(String s, Collector<Item> collector) throws Exception {
                    String[] split = s.split(",");
                    collector.collect(new Item(split[0].trim(), split[1].trim(), split[2].trim(),
                        Double.valueOf(split[3].trim())));
                  }
                })
        )
        .where(
            (KeySelector<PurchaseAnalysis.OrderUser, String>) orderUser -> orderUser.getItemName())
        .equalTo((KeySelector<Item, String>) item -> item.getItemName())
        .window(TumblingProcessingTimeWindows.of(Time.seconds(second)))
        .apply(
            (JoinFunction<PurchaseAnalysis.OrderUser, Item, PurchaseAnalysis.OrderUserItem>) (orderUser, item) -> {
        ...
        .map(new MapFunction<PurchaseAnalysis.OrderUserItem, Tuple2<String, Double>>() {
          @Override
          public Tuple2<String, Double> map(PurchaseAnalysis.OrderUserItem orderUserItem)
              throws Exception {
            return new Tuple2<String, Double>(orderUserItem.getGender(),
                orderUserItem.getItemPrice() * orderUserItem.getQuantity());
          }
        })
        .keyBy(0)
        .reduce(new ReduceFunction<Tuple2<String, Double>>() {
          @Override
          public Tuple2<String, Double> reduce(Tuple2<String, Double> stringDoubleTuple2,
              Tuple2<String, Double> t1) throws Exception {
            return new Tuple2<String, Double>(stringDoubleTuple2.getField(0),
                ((Double) stringDoubleTuple2.getField(1)) + ((Double) t1.getField(1)));
          }
        });
