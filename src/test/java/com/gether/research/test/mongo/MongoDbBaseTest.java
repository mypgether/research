...
public class MongoDbBaseTest {
  ...
  public void before() {
    ...
    mongoClient = new MongoClient(new ServerAddress("localhost", 32769),
  ...
  public void insertObj() {
    ...
    res.forEach((Consumer<? super ProfilePojo>) record -> {
      ...
      System.out.println("title:" + Optional.ofNullable(record.getMap()).map(a -> a.get("title"))
          .map(a -> a.get("supportKey")).orElse(""));
    });
  ...
  public void updateObj() {
    ...
    collection.updateOne(eq("deviceId", pojo.getDeviceId()),
        combine(set("d", "abc_001"), set("map.title", objMap),
            set("createTime", pojo.getCreateTime())),
  ...
  public void distinctTest() {
    ...
    DistinctIterable<String> deviceIdList = collection
        .distinct("deviceId", eq("productKey", "productkey"), String.class);
